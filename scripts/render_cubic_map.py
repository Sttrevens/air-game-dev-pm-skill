...
DONE_STATUSES = {"done", "complete", "completed", "已完成", "完成"}
BLOCKED_STATUSES = {"blocked", "stuck", "阻塞", "卡住"}
IN_PROGRESS_STATUSES = {"in progress", "doing", "wip", "进行中", "开发中"}
ALIASES = {
...
def feature_maturity(row: Dict[str, str]) -> float:
    ...
    if status in BLOCKED_STATUSES:
        return max(0.04, base * 0.55)
    if status in IN_PROGRESS_STATUSES:
        return min(0.98, max(0.06, base + 0.06))
    return base
...
def status_flags(status: str) -> Dict[str, bool]:
    ...
    return {
        "blocked": normalized in BLOCKED_STATUSES,
        "done": normalized in DONE_STATUSES,
        "inProgress": normalized in IN_PROGRESS_STATUSES,
    }
...
def rows_to_mainstays(rows: List[Dict[str, str]]) -> Dict[str, Any]:
    ...
    for mid in sorted(grouped):
        ...
        for row in sorted(grouped[mid], key=lambda r: r["feature_id"]):
            ...
            current_level = LEVEL_SCORE.get(row["current_level"], 0)
            target_level = LEVEL_SCORE.get(row["target_level"], 4)
            flags = status_flags(row["status"])
            features.append(
                {
                    "id": row["feature_id"],
                    ...
                    "targetLevelName": row["target_level"],
                    "targetGap": max(0, target_level - current_level),
...
def numeric_level(value: Any, default: int = 0) -> int:
    if isinstance(value, str):
        return LEVEL_SCORE.get(canonical_level(value), default)
    try:
...
def coerce_bool(value: Any) -> bool:
    if isinstance(value, bool):
        return value
    return str(value or "").strip().lower() in {"1", "true", "yes", "y", "done", "blocked", "已完成", "阻塞"}
...
def normalize_mainstay_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for m in data.get("mainstays", []):
        ...
        for f in m.get("features", []) or []:
            ...
            level_value = numeric_level(f.get("level", f.get("currentLevel", 0)))
            target_level = numeric_level(f.get("targetLevel", f.get("targetLevelName", 4)), 4)
            status = str(f.get("status", "todo"))
            flags = status_flags(status)
            maturity_raw = f.get("maturity", None)
            maturity = float(maturity_raw) if maturity_raw not in (None, "") else level_value / 4
            blocked = coerce_bool(f.get("blocked", flags["blocked"]))
            done = coerce_bool(f.get("done", flags["done"]))
            in_progress = coerce_bool(f.get("inProgress", flags["inProgress"]))
            if blocked:
                maturity = max(0.04, maturity * 0.55)
            features.append(
                {
                    "id": str(f.get("id", "F-?")),
                    ...
                    "level": level_value,
                    "currentLevel": f"L{level_value}",
                    "targetLevel": target_level,
                    "targetLevelName": f"L{target_level}",
                    "targetGap": max(0, target_level - level_value),
...
function clamp(v,a=0,b=1){{return Math.max(a,Math.min(b,v))}}function colorFor(v){{const stops=[new THREE.Color(0xef4444),new THREE.Color(0xf97316),new THREE.Color(0xfacc15),new THREE.Color(0x84cc16),new THREE.Color(0x22c55e)],t=clamp(v),s=t*(stops.length-1),i=Math.min(stops.length-2,Math.floor(s));return stops[i].clone().lerp(stops[i+1],s-i)}}function mat(c,o=1){{return new THREE.MeshStandardMaterial({{color:c,roughness:.58,metalness:.08,transparent:o<1,opacity:o,depthWrite:o>=.92}})}}function block(g,x,y,z,w,h,d,m,meta){{const mesh=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),m);mesh.position.set(x,y+h/2,z);mesh.castShadow=true;mesh.receiveShadow=true;mesh.userData=meta;g.add(mesh);objects.push(mesh);return mesh}}function wireBox(g,x,y,z,w,h,d,color,opacity,meta){{const line=new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(w,h,d)),new THREE.LineBasicMaterial({{color,transparent:true,opacity}}));line.position.set(x,y+h/2,z);line.userData=meta;g.add(line);objects.push(line);return line}}
function labelSprite(text,size=58){{const cvs=document.createElement('canvas'),ctx=cvs.getContext('2d');cvs.width=1024;cvs.height=220;ctx.fillStyle='rgba(4,10,18,.68)';ctx.fillRect(0,0,cvs.width,cvs.height);ctx.strokeStyle='rgba(148,163,184,.24)';ctx.strokeRect(0,0,cvs.width,cvs.height);ctx.font=`760 ${{size}}px Inter,Arial`;ctx.fillStyle='#edf6ff';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(text.slice(0,32),512,110);const tex=new THREE.CanvasTexture(cvs);const spr=new THREE.Sprite(new THREE.SpriteMaterial({{map:tex,transparent:true}}));spr.scale.set(3.5,.75,1);return spr}}function taskMaturity(f,i){{if(!f.tasks||!f.tasks.length)return f.maturity;const t=f.tasks[i];if(!t)return f.maturity;return t.done?Math.max(.88,f.maturity):Math.min(.42,f.maturity)}}function featureRisk(f){{return (f.blocked?2.4:0)+(f.targetGap||0)*.65+(1-(f.maturity||0))*1.4}}
const levelBand=maxHeight/4;
for(let index=0;index<DATA.mainstays.length;index++){{const m=DATA.mainstays[index],c=index%cols,r=Math.floor(index/cols),cx=x0+c*(tile+gap),cz=z0+r*(tile+gap),group=new THREE.Group();group.position.set(cx,0,cz);scene.add(group);mainstayGroups.push({{group,data:m,cx,cz}});block(group,0,0,0,tile,.13,tile,mat(0x182235,.92),{{kind:'mainstay',data:m}});block(group,0,0,0,tile+.04,maxHeight,tile+.04,mat(colorFor(m.maturity),.045),{{kind:'mainstay-shell',data:m}});const edge=new THREE.LineSegments(new THREE.EdgesGeometry(new THREE.BoxGeometry(tile+.05,maxHeight,tile+.05)),new THREE.LineBasicMaterial({{color:colorFor(m.maturity),transparent:true,opacity:.24}}));edge.position.y=maxHeight/2;group.add(edge);const lab=labelSprite(`${{m.id}} ${{m.name}}`,52);lab.position.set(0,.22,tile/2+.36);group.add(lab);labels.push(lab);const features=m.features||[],fcols=Math.ceil(Math.sqrt(Math.max(1,features.length))),frows=Math.ceil(Math.max(1,features.length)/fcols),usable=tile,spacingX=usable/fcols,spacingZ=usable/frows,cellX=spacingX*1.005,cellZ=spacingZ*1.005;for(let i=0;i<features.length;i++){{const f=features[i],fx=-usable/2+(i%fcols)*spacingX+spacingX/2,fz=-usable/2+Math.floor(i/fcols)*spacingZ+spacingZ/2,currentLevel=Math.max(0,Math.min(4,Math.round(f.level||0))),targetLevel=Math.max(0,Math.min(4,Math.round(f.targetLevel??4))),hasTasks=(f.tasks||[]).length>0,targetH=Math.max(.08,targetLevel*levelBand);wireBox(group,fx,0,fz,cellX+.045,targetH,cellZ+.045,0x38bdf8,targetLevel>currentLevel?.46:.22,{{kind:'feature-target',data:f,mainstay:m}});for(let lvl=1;lvl<=4;lvl++){{const active=lvl<=currentLevel,isGap=lvl>currentLevel&&lvl<=targetLevel,tone=f.blocked?Math.min(f.maturity||0,lvl/4):lvl/4,color=active?colorFor(tone):(isGap?new THREE.Color(0xf97316):new THREE.Color(0x334155)),opacity=active?(f.blocked?.76:.98):(isGap?.18:.055),kind=active?'feature':'feature-empty';block(group,fx,(lvl-1)*levelBand,fz,cellX,Math.max(.04,levelBand-.025),cellZ,mat(color,opacity),{{kind,data:f,mainstay:m,level:lvl}})}}const h=Math.max(.08,clamp(f.maturity||0)*maxHeight);if(f.blocked)block(group,fx,h,fz,cellX+.03,.075,cellZ+.03,mat(0xef4444,.95),{{kind:'feature-risk',data:f,mainstay:m}});if(hasTasks){{const tasks=f.tasks,beads=Math.min(tasks.length,8),beadW=Math.max(.06,cellX/beads*.72);for(let s=0;s<beads;s++){{const t=tasks[s],tx=fx-cellX/2+cellX*(s+.5)/beads,ty=Math.max(.18,Math.min(maxHeight-.18,(s+.5)/beads*Math.max(levelBand,h)));block(group,tx,ty,fz+cellZ*.37,beadW,.12,cellZ*.18,mat(t.done?0x22c55e:0xf97316,.96),{{kind:'task',data:f,mainstay:m,task:t}})}}}}}}}}
function esc(s){{return String(s??'').replace(/[&<>"]/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}}[c]))}}function pct(v){{return Math.round((v||0)*100)}}function pctLabel(v){{return `${{pct(v)}}%`}}function level(f){{return `L${{Math.round(f.level||0)}}`}}function targetLevel(f){{return `L${{Math.round(f.targetLevel??4)}}`}}function taskText(done,total){{return total?`${{done}}/${{total}}`:'No task data'}}function taskLabel(done,total){{return total?`${{done}}/${{total}} tasks`:'No task data'}}function totalTasks(m){{return m.features.reduce((a,f)=>a+(f.taskCount||0),0)}}function doneTasks(m){{return m.features.reduce((a,f)=>a+(f.doneTasks||0),0)}}function byWeak(a,b){{return featureRisk(b)-featureRisk(a)}}function summary(m){{const fs=m.features||[],total=fs.length,done=fs.filter(f=>(f.maturity||0)>=.9&&!f.blocked).length,weak=fs.filter(f=>(f.maturity||0)<.45||f.blocked).length,blocked=fs.filter(f=>f.blocked).length,targetGap=fs.reduce((a,f)=>a+(f.targetGap||0),0);return{{total,done,weak,blocked,targetGap,tasks:totalTasks(m),doneTasks:doneTasks(m),avg:pct(m.maturity)}}}}
function taskRow(t){{const done=t.done?'done':'todo';return `<div class="task ${{done}}"><span class="mark"></span><div>${{esc(t.id||'T')}} ${{esc(t.name||'Untitled task')}}<div class="mini">${{esc(t.status||'todo')}}</div></div>${{t.url?`<a href="${{esc(t.url)}}" target="_blank">link</a>`:''}}</div>`}}function featureRow(m,f,clickable=true){{const gap=f.targetGap||0,status=f.blocked?'Blocked':(f.status||'');return `<div class="row" ${{clickable?`data-mid="${{esc(m.id)}}" data-fid="${{esc(f.id)}}"`:''}}><strong>${{esc(f.id)}} ${{esc(f.name)}}</strong><div class="bar"><i style="width:${{pct(f.maturity)}}%"></i></div><div class="meta"><span class="pill">${{level(f)}} -> ${{targetLevel(f)}}</span><span class="pill">${{pctLabel(f.maturity)}}</span><span class="pill">${{gap?`Gap +${{gap}}L`:'On target'}}</span><span class="pill">${{taskLabel(f.doneTasks||0,f.taskCount||0)}}</span><span class="pill">${{esc(status)}}</span></div></div>`}}
function mainstayOverview(m){{const s=summary(m),weak=[...m.features].sort(byWeak).slice(0,5),read=s.blocked?['Clear blocked dependencies first','A blocked pillar should be treated as a red zone even if its current level is not L0.']:(s.targetGap?['Raise the short target gaps before local polish','The next stone is visible, but several Features still need level evidence before deeper production spend.']:(s.weak?['Fill foundations before deep polish','Bring neighboring pillars to L1/L2 evidence before over-investing in the green island.']:['Ready to plan next validation layer','The visible risk is no longer imbalance; choose the next playable or inspectable stone.']));return `<h2>${{esc(m.id)}} ${{esc(m.name)}}</h2><p class="sub">Mainstay is one large pillar; internal blocks are Features. Four fixed height bands mean L1-L4 evidence; blue wireframes mark target level.</p><div class="metric"><div><b>${{s.avg}}%</b><span>Mainstay</span></div><div><b>${{s.total}}</b><span>Features</span></div><div><b>${{taskText(s.doneTasks,s.tasks)}}</b><span>Tasks</span></div><div><b>${{s.blocked}} / ${{s.targetGap}}</b><span>Blocked / Gap</span></div></div><div class="section-title">Features to shore up</div>${{weak.map(f=>featureRow(m,f)).join('')}}<div class="section-title">Air PM read</div><div class="row"><strong>${{esc(read[0])}}</strong><div class="mini">${{esc(read[1])}}</div></div>`}}
function featureDetail(m,f){{const tasks=f.tasks||[],gap=f.targetGap||0;return `<h2>${{esc(f.id)}} ${{esc(f.name)}}</h2><p class="sub">Under ${{esc(m.id)}} ${{esc(m.name)}}.</p><div class="metric"><div><b>${{level(f)}}</b><span>Current</span></div><div><b>${{targetLevel(f)}}</b><span>Target</span></div><div><b>${{pctLabel(f.maturity)}}</b><span>Maturity</span></div><div><b>${{gap?`+${{gap}}L`:'0'}}</b><span>Target Gap</span></div></div><div class="bar"><i style="width:${{pct(f.maturity)}}%"></i></div><div class="meta"><span class="pill">${{esc(f.status||'todo')}}</span><span class="pill">${{taskLabel(f.doneTasks||0,f.taskCount||0)}}</span><span class="pill">${{esc(f.owner||'No owner')}}</span></div><div class="focusline"></div><div class="section-title">Task details</div>${{tasks.length?tasks.map(taskRow).join(''):`<div class="row"><strong>No task rows in this data</strong><div class="mini">CSV mode renders L1-L4 evidence bands. Add task objects in JSON if you want task-level drilldown beads.</div></div>`}}${{f.url?`<div class="row"><strong>Feature link</strong><div class="mini"><a href="${{esc(f.url)}}" target="_blank">Open evidence</a></div></div>`:''}}`}}
function featureList(m){{const rows=[...m.features].sort((a,b)=>featureRisk(b)-featureRisk(a)||a.id.localeCompare(b.id));return `<h2>${{esc(m.id)}} Feature Matrix</h2><input class="search" id="filter" placeholder="Filter features or tasks"/><div id="featureRows">${{rows.map(f=>featureRow(m,f)).join('')}}</div>`}}function gapView(){{const ms=[...DATA.mainstays].sort((a,b)=>(summary(b).blocked-summary(a).blocked)||(summary(b).targetGap-summary(a).targetGap)||((a.maturity||0)-(b.maturity||0))),avg=DATA.mainstays.reduce((a,m)=>a+(m.maturity||0),0)/Math.max(1,DATA.mainstays.length),low=ms.reduce((a,m)=>(m.maturity||0)<(a.maturity||0)?m:a,ms[0]),high=ms.reduce((a,m)=>(m.maturity||0)>(a.maturity||0)?m:a,ms[0]),blocked=DATA.mainstays.reduce((a,m)=>a+summary(m).blocked,0),targetGap=DATA.mainstays.reduce((a,m)=>a+summary(m).targetGap,0);return `<h2>Global Imbalance</h2><p class="sub">Sorts by blocked count, target-level debt, then shortest pillars.</p><div class="metric"><div><b>${{pctLabel(avg)}}</b><span>Average</span></div><div><b>${{pctLabel(low?.maturity)}}</b><span>Lowest</span></div><div><b>${{pctLabel((high?.maturity||0)-(low?.maturity||0))}}</b><span>Height Gap</span></div><div><b>${{blocked}} / ${{targetGap}}</b><span>Blocked / Target</span></div></div>${{ms.map(m=>{{const s=summary(m);return `<div class="row" data-mid="${{esc(m.id)}}"><strong>${{esc(m.id)}} ${{esc(m.name)}} <span class="pill">${{pctLabel(m.maturity)}}</span></strong><div class="bar"><i style="width:${{pct(m.maturity)}}%"></i></div><div class="meta"><span class="pill">${{s.total}} F</span><span class="pill">${{taskLabel(s.doneTasks,s.tasks)}}</span><span class="pill">${{s.blocked}} blocked</span><span class="pill">+${{s.targetGap}}L target</span></div></div>`}}).join('')}}`}}
...
const raycaster=new THREE.Raycaster(),pointer=new THREE.Vector2();function updatePointer(e){{const rect=renderer.domElement.getBoundingClientRect();pointer.x=((e.clientX-rect.left)/rect.width)*2-1;pointer.y=-((e.clientY-rect.top)/rect.height)*2+1}}function hitAt(e){{updatePointer(e);raycaster.setFromCamera(pointer,camera);return raycaster.intersectObjects(objects,false)[0]}}function hover(e){{const hit=hitAt(e);if(!hit){{tip.style.display='none';return}}const u=hit.object.userData,d=u.data;tip.style.display='block';tip.style.left=e.clientX+'px';tip.style.top=e.clientY+'px';if(u.kind==='task')tip.innerHTML=`<b>${{esc(d.id)}} ${{esc(d.name)}}</b><br>${{esc(u.mainstay.id)}} · ${{esc(u.task?.id||'Task')}} · ${{u.task?.done?'Done':'Todo'}}`;else if(u.kind.startsWith('feature'))tip.innerHTML=`<b>${{esc(d.id)}} ${{esc(d.name)}}</b><br>${{esc(u.mainstay.id)}} · ${{level(d)}} -> ${{targetLevel(d)}} · ${{pctLabel(d.maturity)}} · ${{taskLabel(d.doneTasks||0,d.taskCount||0)}}${{d.blocked?'<br>Blocked risk':''}}`;else tip.innerHTML=`<b>${{esc(d.id)}} ${{esc(d.name)}}</b><br>${{pctLabel(d.maturity)}} · ${{(d.features||[]).length}} features · ${{taskLabel(doneTasks(d),totalTasks(d))}}`}}function click(e){{const hit=hitAt(e);if(!hit)return;const u=hit.object.userData;if(u.kind.startsWith('feature')||u.kind==='task')selected={{m:u.mainstay,f:u.data,tab:'tasks'}};else if(u.kind.startsWith('mainstay'))selected={{m:u.data,f:null,tab:'overview'}};render()}}renderer.domElement.addEventListener('pointermove',hover);renderer.domElement.addEventListener('click',click);
